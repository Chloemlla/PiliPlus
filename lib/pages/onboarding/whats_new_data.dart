...
abstract final class WhatsNewData {
  ...
  static List<ImprovementsGuidePageData> get pages => [
    ...
    const ImprovementsGuidePageData(
      ...
      title: 'Synapse 云同步：确认网络环境',
      subtitle: '服务器开启访问验证后，本机会像网页一样请求验证，验证通过才能继续同步。',
      bullets: [
        '干净网络自动通过，不打扰；需要人工确认时才会弹出验证，完成后才能继续同步。',
        '验证结果在本机保留 40 分钟，期间不会反复要求重新验证；过期后静默重新获取。',
        '入口：设置 → 其它设置 →「Synapse 云同步」→「网络访问验证」，可随时主动重新验证。',
    ...
    const ImprovementsGuidePageData(
      ...
      bullets: [
        ...
        '已修回：个人空间视频页的「定位至上次观看」不再锁定内层滚动——该接口同样依赖本分支依赖库里没有的方法。',
      ],
      tip: '以上不影响日常使用。',
    ),
    const ImprovementsGuidePageData(
